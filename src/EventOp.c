/**
* \file EventOp.c
* \brief programme gérant les events
* \author Cognard Luka.
* \version 1.9.5
* \date 16 Avril 2024.
*/
...
void ReactEventsOp(int MouseOver,int KeyIsPressed, int *Action,SDL_Renderer* pRenderer,SDL_Texture* Perso,SDL_Rect marcheHori[3][8],SDL_Rect* dest,int* i,int w,int h,team_t team,int biome,int choiceShop,int* vendeur,int colli[125][250],SDL_Texture * vendeurDialoTexture[5],int *map,SDL_Rect* savedest,int numperso){
     int j, inter=0;
...
void ReactEventsInvStuff(int *yonThrow,int MouseClick,int MouseOver,SDL_Renderer* pRenderer,int w,int h,inventaire_t* inv,SDL_Rect *destThrowButton,team_t * team,int numPage,int numPagePerso,int slot,int* eOuStuff){
	...
	/** regarde si le on est sur des armes */
     		...
     		if(inv->armes[i]!=NULL){
	     		if(inv->armes[i]->persoEquiper!=NULL){
	     			nomObj[i%10]=loadTextureFont("../Ressource/font/BlockKie.ttf",pRenderer,inv->armes[i]->nom,destzone.h,equipedColor);
	     		}
	     		else{
	     			nomObj[i%10]=loadTextureFont("../Ressource/font/BlockKie.ttf",pRenderer,inv->armes[i]->nom,destzone.h,unequipedColor);
	     		}
	     		descObj[i%10]=loadTextureFont("../Ressource/font/BlockKie.ttf",pRenderer,inv->armes[i]->desc,destzone.h,unequipedColor);
	     	}
     	}
     ...
     */
     if(MouseClick!=-1 && MouseClick<(inv->nbEquActu+inv->nbArmActu)){
     	if(MouseClick<inv->nbEquActu){
     		...
     		SDL_RenderCopy(pRenderer,inv->armes[MouseClick]->artwork,NULL,&destImage);
     	}
	...
	/** cas ou l'on a cliquer sur oui */
     ...
     if(*eOuStuff==0){
     	...
     	/*
	 *else if(MouseClick<inv->nbArmActu){
	 * 	equiper_arme   	
	 * }
	 */
     	*eOuStuff=-1;
...
*/
int achat_possible(int choiceBiy,int biome,inventaire_t * inv,objet_t allPot[ALL_OBJETS],equipement_t allStuff[ALL_EQUIPEMENTS][3]){
	if((choiceBiy>-1 && choiceBiy<3) || (choiceBiy>5 && choiceBiy<9)){
		...
		if((choiceBiy>-1 && choiceBiy<3) && biome!=5){
			if(ajouter_stuff(allStuff,inv,choiceBiy+(biome-1)*3)==0){
				...
				if(inv->bourse<allStuff[choiceBiy+(biome-1)*3]->cout){
					return 0;
				}
			}
			else{
		...
		/** cas ou magasin du biome 5 */
			...
			if(ajouter_stuff(allStuff,inv,choiceBiy+biome*3)==0){
				jeter_stuff(inv,inv->nbEquActu-1);
				if(inv->bourse<allStuff[choiceBiy+biome*3]->cout){
					return 0;
				}
			}
			else{
		...
		/** cas ou magasin du biome 4 */
			...
			if(ajouter_stuff(allStuff,inv,choiceBiy-6+biome*3)==0){
				jeter_stuff(inv,inv->nbEquActu-1);
				if(inv->bourse<allStuff[choiceBiy-6+biome*3]->cout){
					return 0;
				}
			}
			else{
	...
	/** objets */
	else if(choiceBiy>2 && choiceBiy<6){
		...
		switch(biome){
			...
			/** biome 1 */
			case 1:
				if(ajouter_obj(inv,allPot,(choiceBiy-3)*3)==0){
					jeter_obj(inv,inv->nbObjActu-1);
					if(inv->bourse<allPot[(choiceBiy-3)*3].cout){
						return 0;
					}
				}
				else{
...
*/
void EventShop(int *MouseOver,int *Action,SDL_Renderer* pRenderer,int biome,equipement_t allStuff[ALL_EQUIPEMENTS][3],inventaire_t* inv,int w,int h,scene_t* scene,objet_t allPot[ALL_OBJETS],int *choiceBiy){
	
	
	/** création de la variable event qui récupère les évènements */
	SDL_Event event;
	int achatPossible=achat_possible(*choiceBiy,biome,inv,allPot,allStuff);
		...
		switch(event.type){
			...
			case SDL_MOUSEBUTTONUP:
				...
				if(biome==1){
					...
					/** item sélectionner */
					if(*MouseOver>=0 && *MouseOver<=4){
						*choiceBiy=*MouseOver;
					}
					
					/** bouton acheter cliquer et un objet est sélectionner */
					else if(*MouseOver==9 && *choiceBiy>-1){
						...
						/** cas d'un objet */
						else if(*choiceBiy>2 && achatPossible){
							ajouter_obj(inv,allPot,(*choiceBiy-3)*3);
							inv->bourse-=allPot[(*choiceBiy-3)*3].cout;
						}
					}
				}
				...
				/** biome 4 */
					...
					/** bouton acheter cliquer et un objet est sélectionner */
						...
						/** cas d'un équipements */
						if(((*choiceBiy>-1 && *choiceBiy<3) || (*choiceBiy>5 && *choiceBiy<9)) && achatPossible){
							if(*choiceBiy>-1 && *choiceBiy<3){
								ajouter_stuff(allStuff,inv,*choiceBiy+(biome-1)*3);
								inv->bourse-=allStuff[*choiceBiy+(biome-1)*3]->cout;
							}
							else{
								ajouter_stuff(allStuff,inv,*choiceBiy-6+biome*3);
								inv->bourse-=allStuff[*choiceBiy-6+biome*3]->cout;
							}
						}
						
						/** cas objet */
						else if((*choiceBiy>2 && *choiceBiy<6) && achatPossible){
							switch(*choiceBiy){
								case 3:
	...
	if(*MouseOver==9 || *choiceBiy==-1 || achatPossible==0){
...
void EventSell(int *MouseOver,int *MouseClick,int *Action,SDL_Renderer* pRenderer,inventaire_t* inv,int w,int h,scene_t* scene,int *numPage){
				...
				/** bouton vendre actionner seulement si un équipement/armes est sélectionner */
					...
					if(*MouseClick<inv->nbEquActu){
				     		...
				     		*MouseClick=-1;
				     		...
				     		inv->bourse+=inv->armes[*MouseClick]->cout/2;
				     		//jeter_arme(inv,*MouseClick);
				     		*MouseClick=-1;
				     	}
				}
	...
	for(i=0+(*numPage*10);i<10+(*numPage*10);i++){
		...
		/** regarde si le on est sur des armes */
	     		...
	     		sprintf(prix,"%d",inv->armes[i]->cout/2);
	     		if(inv->armes[i]!=NULL){
		     		if(inv->armes[i]->persoEquiper!=NULL){
		     			nomObj[i%10]=loadTextureFont("../Ressource/font/BlockKie.ttf",pRenderer,inv->armes[i]->nom,destzone.h,equipedColor);
		     		}
