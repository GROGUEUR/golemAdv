...
/**degats infliges par les mobs*/
int degats_mob(mob_t* mob,attaque_t att,perso_t* perso){
	int calc = (mob->lvl *0.4) + 2;
	calc *= att.qte_effet;
	if(att.type_att == PHY){
		calc *= mob->For;
		calc /=(perso->Def);
	}
	else if(att.type_att == MAG){
		calc *= mob->Int;
		calc /= (perso->Res);
	}
	calc /= 20;
...
void fin_combat(int vict,mob_t * mob,inventaire_t * inv,team_t* team,float statPerso[3][8],scene_t * scene,int* click,int* over){
	int i;
	if(vict == 0){
		inv->bourse += (5 + 2* (mob->lvl));
		for(i=0;i<team->nbPerso;i++){
			team->team[i]->Xp += mob->Xp_don;
			lvl_up(team->team[i],statPerso[i]);
		}
		*scene = OP;
	}
	else if (vict == 1){
		inv->bourse -= inv->bourse / 10;
		for(i=0;i<team->nbPerso;i++){
			team->team[i]->Pv = 1;
		}
		*scene = OP;
...
*/
void DetectEventsFight(int * menu_combat, int* MouseClick,int *MouseOver, int* quit, int* perso,int* obj, scene_t* scene,int w,int h,int * W, int * H,int* atta){
...
*/
void ReactEventsFight(int *p, int perso,int obj, int MouseClick,int MouseOver,int* menu_combat ,SDL_Renderer* pRenderer,SDL_Texture* fond_combat,int w,int h,inventaire_t* inv,team_t * team, int imob,int H, int W,int atta,float statPerso[3][8],scene_t * scene,int* pvMob){
	srand(time(NULL));
	mob_t* mob;
	int mob_lvl = 1;
	if(team->team[0]->lvl > 1){
		mob_lvl = team->team[0]->lvl-1;
	}
	switch(imob){
		case 0:
			mob = creer_mob("Bouffle",pRenderer,mob_lvl);
			if(*pvMob >0)
				mob->Pv = *pvMob;
			break;
		case 1:
			mob = creer_mob("Terminal",pRenderer,mob_lvl);
			if(*pvMob >0)
				mob->Pv = *pvMob;
			break;
	}
	...
	int val,val2;
     ...
     int i,j, textwidth, textheight, nb_pers,degats;
	 ...
	 *p = (*p)%6;
	 ...
	 int dp1 = w/4 - x; /** ecart horizontal a gauche du premier perso et a droite du troisieme perso*/
	 int dp2 = w/4; /** ecart horizontal a droite du premier perso*/
	 int dp3 = (w/2 - y)/2; /** ecart horizontal a gauche et a droite du deuxieme perso*/
	 int dp4 = w/2 - z - dp1; /** ecart horizontal a gauche du troisieme perso*/

	 int l = 300; /**taille du carre du mob*/
	 int lambda = W - l/2; /**ecart vertical au dessus et au dessus du mob*/
	 int mu = (w/2 - l) /2; /**ecart horizonral a gauche et a droite du mob*/

	 int lonpv = x; /**longueur du rectangle du nom des pv*/
	 int larpv = z/2; /**largeur du rectangle du nom des pv*/
	 int dpvh3 = (dp4 - y) /2; /**ecart horizontal a gauche du 3e perso*/
	 int dpvh2 = (dp3 - y) /2; /**ecart horizontal a gauche du 2e perso*/
	 int dpvh1 = (dp2 - y) /2; /**ecart horizontal a gauche du 1er perso*/

	 int dpvv3 = (z - larpv) /2; /**ecart vertical au dessus des pv du 3e perso*/
	 int dpvv2 = (y - larpv) /2; /**ecart vertical au dessus des pv du 2e perso*/
	 int dpvv1 = (x - larpv) /2; /**ecart vertical au dessus des pv du 1er perso*/

	 int dpvm = z/4; /**ecart vertical entre les pv et le mob*/
	...
	int dlar[10] = {dlar1,dlar2,dlar3a,dlar3b,dlar4,dlar5,dlar6,dlar7,dlonRet};
	 ...
	 /** declaration de la couleur du texte */
	 ...
	 SDL_Rect rect_pv_persos[team->nbPerso];
	 SDL_Rect rect_mob = {w/2 + mu,lambda, l,l};

	 SDL_Texture* text_persos[team->nbPerso];
	 SDL_Texture* text_pv_persos[team->nbPerso];
	 SDL_Texture* text_mob = mob->IdleAnim;

	 strcpy(pvs,"PV : ");
	 sprintf(temp,"%.0f",mob->Pv);
	 strcat(pvs,temp);
	 strcat(pvs,"/");
	 sprintf(temp,"%.0f",mob->PvMax);
	 strcat(pvs,temp);
	 SDL_Texture* pvmob = loadTextureFont("../Ressource/font/BlockKie.ttf",pRenderer,pvs,larpv,noir);
	 SDL_Rect pvsmob = {w/2 + mu + larpv,lambda-dpvm-larpv,lonpv,larpv};

	 SDL_Rect IdlAnim[6] = {{0,0,32,32},{32,0,32,32},{32*2,0,32,32},{32*3,0,32,32},{32*4,0,32,32},{32*5,0,32,32}};
	 
	 SDL_RenderCopy(pRenderer,text_mob,&IdlAnim[(*p)],&rect_mob);
	 SDL_RenderCopy(pRenderer,pvmob,NULL,&pvsmob);
	 ...
	 switch(team->nbPerso){ /** chaque 'case' represente le n-ieme perso. S'il y a 3 persos, je cree le rectangle des 3 persos, s'il y en a 2 j'en crée que 2 etc*/
		...
		case 3: 
			...
			rect_pv_persos[2].x = dpvh3;
			rect_pv_persos[2].y = dpvv3 + delta;
			rect_pv_persos[2].w = lonpv;
			rect_pv_persos[2].h = larpv;

			text_persos[2] = team->team[2]->IdleAnim;
			sprintf(temp,"%.0f",team->team[2]->Pv);
			strcat(pvs,temp);
			strcat(pvs,"/");
			sprintf(temp,"%.0f",team->team[2]->PvMax);
			strcat(pvs,temp);
			text_pv_persos[2] = loadTextureFont("../Ressource/font/BlockKie.ttf",pRenderer,pvs,larpv,noir);
			SDL_RenderCopy(pRenderer,text_pv_persos[2],NULL,rect_pv_persos + 2);
			SDL_RenderCopy(pRenderer,text_persos[2],&IdlAnim[(*p)],rect_persos + 2);
		case 2: 
			strcpy(pvs,"PV : ");
			rect_persos[1].x = dp3;
			...
			rect_pv_persos[1].x = dp3 + y + dpvh2;
			rect_pv_persos[1].y = dpvv2 + 2*delta + z;
			rect_pv_persos[1].w = lonpv;
			rect_pv_persos[1].h = larpv;

			text_persos[1] = team->team[1]->IdleAnim;
			sprintf(temp,"%.0f",team->team[1]->Pv);
			strcat(pvs,temp);
			strcat(pvs,"/");
			sprintf(temp,"%.0f",team->team[1]->PvMax);
			strcat(pvs,temp);
			text_pv_persos[1] = loadTextureFont("../Ressource/font/BlockKie.ttf",pRenderer,pvs,larpv,noir);
			SDL_RenderCopy(pRenderer,text_pv_persos[1],NULL,rect_pv_persos + 1);
			SDL_RenderCopy(pRenderer,text_persos[1],&IdlAnim[(*p)],rect_persos + 1);
		case 1: 
			strcpy(pvs,"PV : ");
			rect_persos[0].x = dp1;
			...
			rect_pv_persos[0].x = dp1 + x + dpvh1;
			rect_pv_persos[0].y = dpvv1 + 3*delta + z + y;
			rect_pv_persos[0].w = lonpv;
			rect_pv_persos[0].h = larpv;

			text_persos[0] = team->team[0]->IdleAnim;
			sprintf(temp,"%.0f",team->team[0]->Pv);
			strcat(pvs,temp);
			strcat(pvs,"/");
			sprintf(temp,"%.0f",team->team[0]->PvMax);
			strcat(pvs,temp);
			text_pv_persos[0] = loadTextureFont("../Ressource/font/BlockKie.ttf",pRenderer,pvs,larpv,noir);
			SDL_RenderCopy(pRenderer,text_pv_persos[0],NULL,rect_pv_persos + 0);
			SDL_RenderCopy(pRenderer,text_persos[0],&IdlAnim[(*p)],rect_persos + 0);
	 }
	 ...
	 *p += 1; /** on se decale de une image dans le sprite de l'Idle Animation de tous les persos et mobs*/
			...
			if(team->team[i] != NULL){
				if(team->team[i]->attaques != NULL){
					...
					switch(team->team[i]->attaques[j].type_att){
						...
						case ERR: logos[i][j] = loadTexture("../Ressource/Sprites/Logos/err.png",pRenderer);break;
					}
				}
				else{
	 ...
	 /**lance l'attaque*/
	 if(*menu_combat == 6){
		degats = degats_perso(team->team[perso],team->team[perso]->attaques[atta],mob);
		val = (rand()%100) +1;
		if(val <= team->team[perso]->attaques[atta].precision){ /**calcul precision de l'attaque*/
			mob->Pv -= degats;
			*pvMob = mob->Pv;
		}
		if(mob->Pv <=0){
			fin_combat(0,mob,inv,team,statPerso,scene,&MouseClick,&MouseOver);
		}
		else{
			val = rand()%2;
			val2 = rand()%team->nbPerso;
			degats = degats_mob(mob,mob->attaques[val],team->team[val2]);
			team->team[val2]->Pv -= degats;
		}
		val=1;
		for(i=0;i<team->nbPerso;i++){
			if (team->team[i]->Pv >=0 ){
		...
		if(val == 1){
			...
			fin_combat(val,mob,inv,team,statPerso,scene,&MouseClick,&MouseOver);
		}
		*menu_combat = 0;
	 }
	 ...
	 if(*menu_combat == 7){
	 	if(inv->sac[obj]!= NULL){
			if(inv->sac[obj]->valPv >= 0){
				if(inv->sac[obj]->valMp >= 0){
					if(team->team[perso]->Pv < team->team[perso]->PvMax || team->team[perso]->Mana < team->team[perso]->ManaMax){
						utiliser_obj(inv,team->team[perso],obj);
					}
				}
				else{
					if(team->team[perso]->Pv < team->team[perso]->PvMax){
						utiliser_obj(inv,team->team[perso],obj);
					}
					else{
						*menu_combat = 5;
			...
			else if(inv->sac[obj]->valMp >= 0){
				if(team->team[perso]->Mana < team->team[perso]->ManaMax){
						utiliser_obj(inv,team->team[perso],obj);
				}
				else{
						*menu_combat = 5;
	 ...
	 for(i=0;i<team->nbPerso;i++){
		SDL_DestroyTexture(text_pv_persos[i]);
	 }
	 ...
	 SDL_DestroyTexture(mob->IdleAnim);
	 free(mob);
	 mob=NULL;
     /** delais d'affichage */
     SDL_Delay(35);
